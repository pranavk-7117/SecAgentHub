...
Architecture:
  RemediationCandidate
    ...
    → Compliance Control Re-check
    → Regression Check
    → ValidationResult (PASSED / FAILED)

The AI has ZERO say in whether the fix passes.
Only deterministic engines (Checkov, graph analysis, compliance mapping) decide.
"""
...
def verify_remediation(
    ...
    Steps:
        1. HCL Validation
        2. Checkov Re-scan
        3. Attack Graph Rebuild
        4. Attack Path Comparison
        5. Compliance Control Re-check
        6. Regression Check
    """
    ...
    steps.append(
        ValidationStep(
            ...
            detail=(
                hcl_result.error_message
                or f"Valid HCL with {hcl_result.resource_count} resources"
            ),
        )
    )
    ...
    if not hcl_result.valid:
        failure_reasons.append(
            f"HCL validation failed: {hcl_result.error_message}"
        )
        ...
        return ValidationResult(
            ...
            compliance_verification=ComplianceVerificationResult(
                passed=False,
                detail="Compliance check skipped because HCL validation failed",
            ),
    ...
    # -----------------------------------------------------------------------
    new_findings = run_checkov(
        candidate.terraform_patch,
        "patched.tf",
    )

    checkov_result = _compare_checkov(
        original_findings,
        new_findings,
        candidate.target_check_ids,
    )
    ...
    steps.append(
        ValidationStep(
            ...
            status=(
                "passed"
                if checkov_result.failed_count
                <= checkov_result.original_failed_count
                else "failed"
            ),
            detail=(
                f"Failed: {checkov_result.original_failed_count} → "
                f"{checkov_result.failed_count}, "
    ...
    steps.append(
        ValidationStep(
            ...
            status="passed",
            detail=(
                f"Rebuilt attack graph with "
                f"{len(new_graph.get('nodes', []))} nodes and "
                f"{len(new_graph.get('edges', []))} edges"
            ),
        )
    )
    ...
    # -----------------------------------------------------------------------
    path_comparison = _compare_attack_paths(
        original_graph,
        new_graph,
    )

    paths_improved = (
        path_comparison.attack_paths_after
        <= path_comparison.attack_paths_before
    )
    ...
    steps.append(
        ValidationStep(
            ...
            detail=(
                f"Paths: {path_comparison.attack_paths_before} → "
                f"{path_comparison.attack_paths_after}, "
    ...
    if not paths_improved:
        failure_reasons.append(
            f"Attack paths increased from "
            f"{path_comparison.attack_paths_before} "
            f"to {path_comparison.attack_paths_after}"
        )

    # -----------------------------------------------------------------------
    # Step 5: Compliance Control Re-check
    # -----------------------------------------------------------------------
    compliance_result = _verify_compliance(
    ...
    steps.append(
        ValidationStep(
            ...
            detail=compliance_result.detail,
        )
    )

    if not compliance_result.passed:
        failure_reasons.append(
            f"Compliance verification failed: "
            f"{compliance_result.detail}"
        )
    ...
    # -----------------------------------------------------------------------
    regression_passed = (
        len(checkov_result.new_findings) == 0
        and len(path_comparison.new_paths) == 0
    )
        ...
        ValidationStep(
            ...
            detail=(
                ...
                if regression_passed
                    ...
                    f"Regression: "
                    f"{len(checkov_result.new_findings)} new findings, "
                    f"{len(path_comparison.new_paths)} new paths"
                )
            ),
        )
    )
    ...
    if not regression_passed:
        if checkov_result.new_findings:
            failure_reasons.append(
                "New Checkov findings introduced: "
                + ", ".join(checkov_result.new_findings)
            )

        if path_comparison.new_paths:
            failure_reasons.append(
                f"New attack paths introduced: "
                f"{len(path_comparison.new_paths)}"
            )
    ...
    critical_regression = any(
        "CRITICAL" in str(f).upper()
        or "HIGH" in str(f).upper()
        for f in checkov_result.new_findings
    )

    # Compliance is now part of the deterministic final decision.
    overall_passed = (
        hcl_result.valid
        and overall_improved
        and compliance_result.passed
        and not critical_regression
    )

    if not overall_passed and not failure_reasons:
        failure_reasons.append(
            "No measurable security improvement detected"
        )
    ...
    try:
        ...
        if resource_count == 0 and "resource" in patched_hcl:
            return HCLValidationResult(
                valid=True,
                error_message=(
                    "Warning: parser found 0 resources "
                    "but HCL contains resource blocks"
                ),
                resource_count=0,
            )
...
def _compare_checkov(
    ...
    for check in (
        original_findings.get("results") or {}
    ).get("failed_checks", []):
    ...
    original_summary = summarize_findings(
        original_findings
    )

    new_summary = summarize_findings(
        new_findings
    )
    ...
    return CheckovRescanResult(
        ...
        original_failed_count=original_summary.get(
            "failed_count",
            0,
        ),
...
def _compare_attack_paths(
    ...
    original_paths = original_graph.get(
        "critical_attack_paths",
        [],
    )

    new_paths = new_graph.get(
        "critical_attack_paths",
        [],
    )
    ...
    def _is_critical_path(
        ...
        return any(
            "s3" in node.lower()
            or "rds" in node.lower()
            or "secret" in node.lower()
            for node in path
        )

    original_critical = sum(
        1
        for p in original_paths
        if _is_critical_path(p)
    )
    ...
    return AttackPathComparison(
        ...
        blast_radius_before=original_graph.get(
            "blast_radius_score",
            0,
        ),
        blast_radius_after=new_graph.get(
            "blast_radius_score",
            0,
        ),
    )
...
def _verify_compliance(
    ...
    """Verify that mapped compliance controls are resolved."""
    ...
    # Controls attached to the original targeted findings
    for check_id in original_failed & targeted_ids:
        for control in get_compliance_controls(check_id):
            ...
            if framework and control_id:
                original_controls.add(
                    f"{framework}:{control_id}"
                )

    # Controls still attached to findings after remediation
    for check_id in new_failed & targeted_ids:
        for control in get_compliance_controls(check_id):
            ...
            if framework and control_id:
                remaining_controls.add(
                    f"{framework}:{control_id}"
                )

    resolved_controls = (
        original_controls - remaining_controls
    )

    # No mapping exists for this finding.
    # Do not fail the remediation just because the
    # compliance layer has no mapping for it yet.
    if not original_controls:
        return ComplianceVerificationResult(
            passed=True,
            detail=(
                "No mapped compliance controls "
                "for targeted findings"
            ),
        )

    # Some controls still remain.
    if remaining_controls:
        remaining = ", ".join(
            sorted(remaining_controls)
        )

        resolved = (
            ", ".join(sorted(resolved_controls))
            or "None"
        )

        return ComplianceVerificationResult(
            original_controls=sorted(original_controls),
            resolved_controls=sorted(resolved_controls),
            remaining_controls=sorted(remaining_controls),
            passed=False,
            detail=(
                f"Resolved: {resolved}; "
    ...
    # All mapped controls resolved.
    resolved = ", ".join(
        sorted(resolved_controls)
    )
    ...
    return ComplianceVerificationResult(
        ...
        passed=True,
        detail=(
            f"All mapped compliance controls resolved: "
            f"{resolved}"
        ),
    )
