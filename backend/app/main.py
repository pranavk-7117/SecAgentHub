...
@app.get("/api/v1/scan/{scan_id}/attack-paths")
async def get_attack_paths(scan_id: str, user_id: str = Depends(get_current_user_id)):
    scan = repository.get_scan(scan_id, user_id=user_id)
    if scan is None:
        raise HTTPException(status_code=404, detail="Scan not found.")
    return {
        "scan_id": scan_id,
        "attack_paths": scan.graph.get("attack_paths", []),
        "security_twin": scan.graph.get("security_twin", {}),
        "security_gate": scan.graph.get("security_gate", {}),
    }
